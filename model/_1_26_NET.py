...
class BoostBlock(nn.Module):
    ...
    def __init__(self,inplanes=1024,axu_inplanes =128, index=0,mode = 'cat',num=5):
        ...
        self.conv2d = ConvBNR(inplanes, inplanes, 3)
...
class Net(nn.Module):
    def __init__(self,path=None):
        ...
        self.boost_2 = BoostBlock(inplanes=1024,axu_inplanes=1024,index=1,mode='cat')
        self.boost_3 = BoostBlock(inplanes=512,axu_inplanes=2048,index=2,mode='cat')
        self.reduce_3 = Conv1x1(2048,1024)
        self.boost_4 = BoostBlock(inplanes=256,axu_inplanes=1024,index=3,mode='cat')
        self.reduce_2 = Conv1x1(1024,512)
        self.boost_5 = BoostBlock(inplanes=128,axu_inplanes=512,index=4)
    ...
    def forward(self,x):
        ...
        mainflow_4 = self.boost_2(mainflow,x4) #B*2048*12*12
        mainflow_3 = self.boost_3(x3,mainflow_4) #B*1024*24*24
        reduce3 = self.reduce_3(mainflow_4)
        # att = F.interpolate(att, c.size()[2:], mode='bilinear', align_corners=False)
        reduce3 = F.interpolate(reduce3, mainflow_3.size()[2:], mode='bilinear', align_corners=False)#upsample
        mainflow_3 = mainflow_3 +  reduce3
        mainflow_2 = self.boost_4(x2,mainflow_3 ) #B*512*48*48
        reduce2 = self.reduce_2(mainflow_3)
        reduce2 = F.interpolate(reduce2, mainflow_2.size()[2:], mode='bilinear', align_corners=False)  # upsampl
        mainflow_2 = mainflow_2 + reduce2
        edge_out, pred1, pred2 = self.boost_5(x1,mainflow_2)
        ...
        edge_out = F.interpolate(edge_out, scale_factor=8, mode='bilinear', align_corners=False)
        edge = F.interpolate(edge, scale_factor=4, mode='bilinear', align_corners=False)
        return pred1,pred2,edge_out,edge
