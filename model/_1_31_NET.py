...
class BoostBlock(nn.Module):
    ...
    def __init__(self, inplanes_s=256,inplanes_r = 512,pre_planes=256,planes=128):
        ...
        self.sft = nn.Sequential(
                ConvBNR(planes+planes,planes+planes,3),
                Conv1x1(planes+planes,planes)
            )
    ...
    def forward(self, s,r,edge,flow = None):
        ...
        if fdm_r_out.size()[2:] != fdm_s_out.size()[2:]:
            fdm_r_out = F.interpolate(fdm_r_out, size=fdm_s_out.size()[2:], mode='bilinear', align_corners=False)
        r_s = torch.cat((fdm_r_out,fdm_s_out),dim=1)
        out = self.sft(r_s)
        return out
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.boost2 = BoostBlock(inplanes_s=512,inplanes_r=1024,pre_planes=256,planes=128)
        self.boost3 = BoostBlock(inplanes_s=256,inplanes_r=512,pre_planes=128,planes=128)
        self.boost4 = BoostBlock(inplanes_s=128,inplanes_r=256,pre_planes=128,planes=128)
